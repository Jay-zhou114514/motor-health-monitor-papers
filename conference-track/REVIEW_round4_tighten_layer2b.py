#!/usr/bin/env python3
"""Layer-2 tightening, pass 2: the caption target must not span a line-broken "67–" token."""
...
paper = sub_once(
    ...
    "condition for two detectors, by 10–45% and 14–68%; under between-fold variation the preregistered "
    "80% criterion is met in neither, at 2 of 6 and 4 of 6, and the third detector saturated at 67–"
    "100% false alarms. We therefore report detector stability in this setting as definition-dependent "
    "rather than as an established improvement.",
    "conclusion",
)

paper = sub_once(
    paper,
    "the effect holds for 3σ RMS and Isolation Forest (6 of 6 operating conditions each under the "
    "pooled SD definition, 4 of 6 and 2 of 6 under the between-fold definition) but not for the "
    "Mahalanobis detector, which saturates at 67–",
    "the effect holds for 3σ RMS and Isolation Forest under the pooled SD definition (6 of 6 "
    "conditions) but not under between-fold variation (4 of 6 and 2 of 6), so the preregistered 80% "
    "stability criterion holds only under the pooled reading; the Mahalanobis detector saturates at 67–",
    "figure caption",
)
